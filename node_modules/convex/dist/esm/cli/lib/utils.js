...
export function fetchDeprecationCheckWarning(ctx, resp) {
  const headers = resp.headers;
  if (headers) {
    const deprecationState = headers.get("x-convex-deprecation-state");
    const deprecationMessage = headers.get("x-convex-deprecation-message");
...
export function deploymentFetch(deploymentUrl, onError) {
  return (resource, options) => {
    ...
    const func = throwingFetch(url, {
      retries: 6,
      retryDelay,
      retryOn: function(_attempt, error, response) {
        if (onError) {
          onError(error);
        }
        if (error) {
        ...
        if (response && !response.ok && method && IDEMPOTENT_METHODS.includes(method)) {
          if ([
            400,
            // Bad Request
            401,
...
export function deploymentClient(deploymentURL, onError) {
